...
document.addEventListener('DOMContentLoaded', function () {
    ...
    chrome.storage.local.get({ savedTexts: [] }, function (data) {
        const savedTexts = Array.isArray(data.savedTexts) ? data.savedTexts : [data.savedTexts];
        savedTexts.forEach(text => {
            textarea.value += text + "\n";
        });
        document.querySelector(".comment_length").textContent = textarea.value.length;
    });

    chrome.storage.local.get({savedTitle: "메모명" }, function(data) {
        title.value = data.savedTitle
        document.querySelector(".title_length").textContent = title.value.length || '3';
    })
    ...
    if (saveMemoButton) {
        saveMemoButton.onclick = function() {
            ...
            //내용 소켓 통신으로 보내기
            chrome.runtime.sendMessage({ action: "saveMemo", title: memoTitle, content:content });
    ...
    if (textarea) {
        textarea.addEventListener("input", function () {
            ...
            // 글자수 세기
            document.querySelector(".comment_length").textContent = this.value.length || '0';
    ...
    if (title) {
        title.addEventListener("input", function () {
            ...
            document.querySelector(".title_length").textContent = this.value.length || '0';
